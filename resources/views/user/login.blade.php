...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        html,body{background:#000;color:var(--text);font-family:'DM Sans',sans-serif;min-height:100vh;overflow:hidden;}
        ...
        .logo-block{text-align:center;margin-bottom:40px;animation:fadeInDown .6s ease forwards;}
        .logo-icon{width:64px;height:64px;border:1px solid var(--border-hot);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;position:relative;clip-path:polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px));animation:pulse-glow 3s ease-in-out infinite;}
        .logo-icon svg{width:28px;height:28px;stroke:var(--green);fill:none;filter:drop-shadow(0 0 8px var(--green));}
        ...
        .logo-name{font-family:'Black Ops One',cursive;font-size:2rem;color:var(--green);text-shadow:0 0 15px var(--green),0 0 40px rgba(170,255,0,.3);letter-spacing:4px;}
        .logo-sub{font-family:'Share Tech Mono',monospace;font-size:.6rem;color:var(--text-dim);letter-spacing:4px;text-transform:uppercase;margin-top:4px;}
        ...
        .card-title{font-family:'Syne',sans-serif;font-weight:700;font-size:.65rem;color:var(--green);letter-spacing:4px;text-transform:uppercase;margin-bottom:24px;display:flex;align-items:center;gap:8px;}
        ...
        .err{border:1px solid #ff4444;background:rgba(255,68,68,.05);color:#ff4444;font-family:'Share Tech Mono',monospace;font-size:.65rem;letter-spacing:1px;padding:10px 14px;margin-bottom:18px;display:none;}
        ...
        .corner{position:fixed;font-family:'Share Tech Mono',monospace;font-size:.5rem;color:var(--text-dim);letter-spacing:1px;z-index:3;}
...
<body>
<div class="corner corner-tl">SYS:USER_AUTH v1.0</div>
<div class="corner corner-br">CURRENSEE // SECURE</div>
...
<div class="wrap">
    <div class="logo-block">
        ...
        <div class="logo-sub">ACCESS TERMINAL<span class="cursor">_</span></div>
    </div>
    ...
    <div class="card">
        ...
        <div class="field">
            ...
            <input type="password" id="password" class="inp" placeholder="••••••••••••" autocomplete="current-password">
        </div>
        <button class="btn" id="submit-btn" onclick="doLogin()">INITIATE ACCESS</button>
    </div>

    <div class="bottom-tag">
        NO ACCOUNT? <a href="/app/register">REGISTER ACCESS</a>
    </div>
</div>
...
<script>
    ...
    async function doLogin() {
        ...
        if (!email || !password) { err.textContent = '> ALL FIELDS REQUIRED'; err.style.display = 'block'; return; }
        ...
        try {
            ...
            if (!res.ok) { err.textContent = '> ' + (data.message || 'AUTHENTICATION FAILED'); err.style.display = 'block'; btn.disabled=false; btn.textContent='INITIATE ACCESS'; return; }
        ...
        } catch(e) { err.textContent = '> CONNECTION ERROR'; err.style.display='block'; btn.disabled=false; btn.textContent='INITIATE ACCESS'; }
    }
